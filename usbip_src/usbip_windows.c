...
int usbip_vbus_attach_device(HANDLE fd, int port, struct usb_device *udev,
		struct usb_interface *uinf0, char *host, char *busid)
{
	int ret;
	...
	plugin.addr = port;
	swprintf(plugin.host, sizeof(plugin.host), L"%s", CharToWchar(host));
	swprintf(plugin.busid, sizeof(plugin.host), L"%s", CharToWchar(busid));
...
static int import_device(int sockfd, struct usb_device *udev,
		...
		HANDLE *devfd, char *host, char *busid)
{
	HANDLE fd;
	int port, ret;
	...
	ret = usbip_vbus_attach_device(fd, port, udev, uinf0, host, busid);
...
static int query_import_device(int sockfd, char *busid,
		struct usb_interface *uinf0, HANDLE * fd, char *host)
{
	int ret;
	...
	/* import a device */
	return import_device(sockfd, &reply.udev, uinf0, fd, host, busid);
}
...
int attach_device(char * host, char * busid)
{
	...
	rhport = query_import_device(sockfd, busid, &uinf, &devfd, host);
	if (rhport < 0) {
		err("query");
		return 0;
	}
